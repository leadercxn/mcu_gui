...
#ifdef SPI_MASTER
  /* Initialize push-buttons mounted on FT32F030XX-EVAL board */
  FT_EVAL_PBInit(BUTTON_RIGHT, BUTTON_MODE_GPIO);
  FT_EVAL_PBInit(BUTTON_LEFT, BUTTON_MODE_GPIO);
  FT_EVAL_PBInit(BUTTON_UP, BUTTON_MODE_GPIO);
  FT_EVAL_PBInit(BUTTON_DOWN, BUTTON_MODE_GPIO);
  FT_EVAL_PBInit(BUTTON_SEL, BUTTON_MODE_GPIO);
...
static void SPI_Config(void)
{
  ...
  SPI_InitStructure.SPI_NSS = SPI_NSS_Hard;//SPI_NSS_Soft;//SPI_NSS_Hard;
...
{
  /* ---------------------------------------------------------------------------
  TIM3 Configuration: generate 1 PWM signals with 50% duty cycle.
    ...
    The TIM3 is running at 4 KHz: TIM3 Frequency = TIM3 counter clock/(TIM3_ARR + 1)
                                                  = 48 MHz / (TIM3_ARR +1) = 4 KHz
                                                 ==> TIM3_ARR + 1 = 12000
    TIM3 Channel4 duty cycle = (TIM3_CCR4/ TIM3_ARR)* 100 = 50%
    ==> TIM3_CCR4 = TIM3_ARR/2 = 6000             
  ----------------------------------------------------------------------------*/
...
static JOYState_TypeDef Read_Joystick(void)
{
  ...
  if (FT_EVAL_PBGetState(BUTTON_RIGHT))
  {
    while (FT_EVAL_PBGetState(BUTTON_RIGHT) == RESET)
    {}
    return JOY_RIGHT;
  }
  /* "JOY_LEFT" key is pressed */
  if (FT_EVAL_PBGetState(BUTTON_LEFT))
  {
    while (FT_EVAL_PBGetState(BUTTON_LEFT) == RESET)
    {}
    return JOY_LEFT;
  }
  /* "JOY_UP" key is pressed */
  if (FT_EVAL_PBGetState(BUTTON_UP))
  {
    while (FT_EVAL_PBGetState(BUTTON_UP) == RESET)
    {}
    return JOY_UP;
  }
  /* "JOY_DOWN" key is pressed */
  if (FT_EVAL_PBGetState(BUTTON_DOWN))
  {
    while (FT_EVAL_PBGetState(BUTTON_DOWN) == RESET)
    {}
    return JOY_DOWN;
  }
  /* "JOY_SEL" key is pressed */
  if (FT_EVAL_PBGetState(BUTTON_SEL))
  {
    while (FT_EVAL_PBGetState(BUTTON_SEL) == RESET)
    {}
    return JOY_SEL;
  }
  /* No key is pressed */
